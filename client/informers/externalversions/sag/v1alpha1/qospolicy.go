...
func NewQosPolicyInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredQosPolicyInformer(client, namespace, resyncPeriod, indexers, nil)
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.SagV1alpha1().QosPolicies(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.SagV1alpha1().QosPolicies(namespace).Watch(context.TODO(), options)
			},
		},
		&sagv1alpha1.QosPolicy{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *qosPolicyInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredQosPolicyInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *qosPolicyInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&sagv1alpha1.QosPolicy{}, f.defaultInformer)
}

func (f *qosPolicyInformer) Lister() v1alpha1.QosPolicyLister {
	return v1alpha1.NewQosPolicyLister(f.Informer().GetIndexer())
}
