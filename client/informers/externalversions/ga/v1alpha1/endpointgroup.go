...
func NewEndpointGroupInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredEndpointGroupInformer(client, namespace, resyncPeriod, indexers, nil)
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.GaV1alpha1().EndpointGroups(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.GaV1alpha1().EndpointGroups(namespace).Watch(context.TODO(), options)
			},
		},
		&gav1alpha1.EndpointGroup{},
		resyncPeriod,
		indexers,
	)
}

func (f *endpointGroupInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredEndpointGroupInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *endpointGroupInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&gav1alpha1.EndpointGroup{}, f.defaultInformer)
}
