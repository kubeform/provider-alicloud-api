...
type BgpPeerLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.BgpPeer, err error)
...
// List lists all BgpPeers in the indexer.
func (s *bgpPeerLister) List(selector labels.Selector) (ret []*v1alpha1.BgpPeer, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.BgpPeer))
	})
	return ret, err
}
...
type BgpPeerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.BgpPeer, error)
...
// List lists all BgpPeers in the indexer for a given namespace.
func (s bgpPeerNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.BgpPeer, err error) {
...
// Get retrieves the BgpPeer from the indexer for a given namespace and name.
func (s bgpPeerNamespaceLister) Get(name string) (*v1alpha1.BgpPeer, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("bgppeer"), name)
	}
	return obj.(*v1alpha1.BgpPeer), nil
}
